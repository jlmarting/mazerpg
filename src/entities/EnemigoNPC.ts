...
export class EnemigoNPC extends EntidadRPG {
  id: number;
  tipo: string;
  ultimaVezActuadoIA: number;
  radioDeVisionIA: number;
  ...
  actualizarIA(game: any) {
    ...
    game.jugadoresRemotos.forEach((j: any) => {
  ...
  puedeAtravesar(deltaFila: number, deltaColumna: number, game: any) {
    ...
    if (sigFila < 0 || sigFila >= game.NUMERO_FILAS || sigColumna < 0 || sigColumna >= game.NUMERO_COLUMNAS) return false;
  ...
  ejecutarMovimientoIA(deltaFila: number, deltaColumna: number, game: any) {
    ...
    if (game.esHost && game.multiplayerActivo) {
      game.enviarMensaje({
        tipo: 'npc_update',
        id: this.id,
        f: this.fila,
  ...
  dibujar(ctx: CanvasRenderingContext2D, offset: { colOffset: number, filaOffset: number }, config: any) {
    ...
    const { TAMANO_CELDA, ALTO_UI_TOP, CELDAS_VISIBLES_X, CELDAS_VISIBLES_Y, TIEMPO_DESVANECIMIENTO_NIEBLA, vistaDebugActivada, mapaLaberinto } = config;
